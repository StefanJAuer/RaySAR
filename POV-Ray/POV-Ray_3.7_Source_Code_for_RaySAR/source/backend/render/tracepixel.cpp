	...
	switch(camera.Type)
	{
		...
		case ORTHOGRAPHIC_CAMERA:
			...
			if(Trace_Level == 1){
				...
				Assign_Vector(Pixel_Initial_Radar, ray.Origin);

				DBL Azimuth_Step, Elevation_Step;
				VLength(Azimuth_Step,*cameraRight);
				VLength(Elevation_Step,*cameraUp);
				...
				Trace_Level++;  // Deactivate if request for this ray
			}
