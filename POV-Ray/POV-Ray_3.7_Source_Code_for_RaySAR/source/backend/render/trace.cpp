...
Trace::Trace(boost::shared_ptr<SceneData> sd, TraceThreadData *td, unsigned int qf,
	...
	Reflectivity_Bounce(1.0),
	Reflectivity_Bounce_Temp(0.0),
	Total_Depth(0.0),
	Trace_Level(1.0)
{
	...
	// new for RaySAR simulation
	if (SAR_Output_Data_Flag == 1.0)
	{
		refWriter = contribWriter.getInstance();
	}
}
...
double Trace::TraceRay(const Ray& ray, Colour& colour, COLC weight, TraceTicket& ticket, bool continuedRay, DBL maxDepth)
{
	...
	if(traceLevelIncremented)
	{
		...
		// New for RaySAR simulator
		Trace_Level = ticket.traceLevel;  // Store current trace level in global variable
	}
...
void Trace::ComputeTextureColour(Intersection& isect, Colour& colour, const Ray& ray, COLC weight, bool photonPass, TraceTicket& ticket)
{
	...
	Total_Depth += isect.Depth;  // new for RaySAR
                                  ...
                                  const Vector3d& rawnormal, const Ray& ray, COLC weight, Intersection& isect, TraceTicket& ticket)
{
	...
	for(layer_number = 0, layer = texture; (layer != NULL) && (trans > ticket.adcBailout); layer_number++, layer = reinterpret_cast<const TEXTURE *>(layer->Next))
	{
		...
		else
		{
			...
			Reflectivity_Bounce_Temp = listWNRX->back().reflec.blue();
			// New for RaySAR Simulator
	...
	if (SAR_Output_Data_Flag == 1.0){
		...
		Camera cam = sceneData->parsedCamera;
		...
		switch(cam.Type)
		{
			case ORTHOGRAPHIC_CAMERA:	
				...
				if(ticket.traceLevel > 1){
					...
					straight_intersection(IPoint_Plane,isect.IPoint,Direction_Radar,Pixel_Initial_Radar,Direction_Primary_Ray);

					// Calculate spatial distance between intersection point (within image plane) and current object point
					VDist(Distance_Radar,IPoint_Plane,isect.IPoint);
					...
					C[0]=cam.Location[0];C[1]=cam.Location[2];C[2]=cam.Location[1];
					I[0]=IPoint_Plane[0]; I[1]=IPoint_Plane[2]; I[2]=IPoint_Plane[1];
					L[0]=cam.Look_At[0]; L[1]=cam.Look_At[2]; L[2]=cam.Look_At[1];
               ...
               refWriter->add(Azimuth,Slant_Range,Elevation,Intensity_Radar*Reflectivity_Bounce,ticket.traceLevel,mark_specular,isect.IPoint[0],isect.IPoint[1],isect.IPoint[2]);
               /*	fprintf (pFile_A, "%f %f %f %f %d %d %f %f %f \n",Azimuth,Slant_Range,Elevation,Intensity_Radar*Reflectivity_Bounce,ticket.traceLevel,mark_specular,isect.IPoint[0],isect.IPoint[1],isect.IPoint[2]); */
				}
				else{

					if (Intensity_Radar > 10e-6){

                  refWriter->add(Azimuth,Slant_Range,Elevation,Intensity_Radar*Reflectivity_Bounce,ticket.traceLevel,mark_specular);                  					    
                  /* fprintf (pFile_A, "%f %f %f %f %d %d \n",Azimuth,Slant_Range,Elevation,Intensity_Radar*Reflectivity_Bounce,ticket.traceLevel,mark_specular); */ 
					}
				}
                                 ...
                                 const RGBColour& layer_pigment_colour, double attenuation, bool backside)
{
	...
	if (SAR_Output_Data_Flag == 1.0){
		...
		n = dot(Vector3d(Direction_Current_Ray), layer_normal); // --> result: cos(a)

		// Scale calculated cosine by minus 2
		n *= -2.0; // --> cos(a)*(-2)

		VECTOR layer_normal_temp;
		layer_normal_temp[0] = layer_normal[0];
		layer_normal_temp[1] = layer_normal[1];
		layer_normal_temp[2] = layer_normal[2];
		
		// Calcualte ray in specular direction
		VAddScaled(Specular_Ray, Direction_Current_Ray, n, layer_normal_temp);

		// Calculate cosine between new ray and vector pointing to light source
		Angle_Diff_Radar = dot(Vector3d(lightsourceray.Direction),Vector3d(Specular_Ray)); // --> cos(b)
