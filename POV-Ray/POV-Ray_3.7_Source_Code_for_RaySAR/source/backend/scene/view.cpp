...
void View::StartRender(POVMS_Object& renderOptions)
{
	...
	{
		// Start contributions writer
		contribWriter.getInstance()->start();
	}
...
void View::RenderControlThread()
{
	...
	while(stopRequsted == false)
	{
		...
		// new for RaySAR simulation, write reflection contributions to file
		if((renderTasks.IsDone() == true) && (renderTasks.Failed() == false) && (SAR_Output_Data_Flag == 1.0))
		{
			//contribWriter.getInstance()->writeToFile();
			contribWriter.getInstance()->finish();
		}
